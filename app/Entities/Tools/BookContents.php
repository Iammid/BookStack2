...
class BookContents
{
    ...
    {
        $chapters = $this->book->chapters()->with('childChapters')->scopes('visible')->get();
        $chapterMap = $chapters->keyBy('id');

        // Initialize an empty collection for the tree
        $tree = collect();

        // Iterate through chapters and assign children
        foreach ($chapters as $chapter) {
            if ($chapter->parent_chapter_id) {
                $parent = $chapterMap->get($chapter->parent_chapter_id);
                if ($parent) {
        ...
        // Fetch only top-level pages (not associated with any chapter)
        $pages = $this->getPages($showDrafts, $renderPages);

        // Attach top-level pages directly to the tree
        foreach ($pages as $page) {
            $tree->push($page);
        }

        // Sort the tree based on priority
        $tree = $tree->sortBy('priority');
    ...
    protected function getPages(bool $showDrafts = false, bool $getPageContent = false): Collection
    {
        ...
        $query->where('book_id', '=', $this->book->id)
              ->whereNull('chapter_id'); // Only top-level pages
