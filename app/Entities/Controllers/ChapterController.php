...
class ChapterController extends Controller
{
     ...
     */
    public function create(string $bookSlug, ?string $parentChapterSlug = null)
    {
        $book = $this->entityQueries->books->findVisibleBySlugOrFail($bookSlug);
        $parentChapter = $parentChapterSlug ? $this->queries->findVisibleBySlugsOrFail($bookSlug, $parentChapterSlug) : null;
     ...
     */
    public function store(Request $request, string $bookSlug, ?string $parentChapterSlug = null)
    {
        $validated = $this->validate($request, [
            'name' => ['required', 'string', 'max:255'],
            'description_html' => ['nullable', 'string', 'max:2000'],
        ]);
    
        $book = $this->entityQueries->books->findVisibleBySlugOrFail($bookSlug);
        $parentChapter = $parentChapterSlug ? $this->queries->findVisibleBySlugsOrFail($bookSlug, $parentChapterSlug) : null;
    
        $validated['parent_chapter_id'] = $parentChapter ? $parentChapter->id : null;
    ...
    public function show(string $bookSlug, string $chapterSlug)
    {
        ...
        $childChapters = $chapter->childChapters()->with('childChapters')->get(); // Eager load deeper levels
        ...
        return view('chapters.show', [
            ...
            'pages'          => $pages,
            'childChapters'  => $childChapters, // Pass child chapters to the view
