...
abstract class Entity extends Model implements Sluggable, Favouritable, Viewable, Deletable, Loggable
{
     ...
     */
    public function matchesOrContains($entity): bool
    {
        // Direct match check
        ...
        // Check for child chapters recursively if this is a chapter
        if (method_exists($this, 'childChapters')) {
            foreach ($this->childChapters as $childChapter) {
                if ($childChapter->matchesOrContains($entity)) {
                    return true;
                }
            }
        }
    
        // Check for contained pages if this is a chapter
        ...
        // Check for other types of children (if needed)
        if (method_exists($this, 'bookChildren')) {
            foreach ($this->bookChildren as $child) {
                if ($child->matchesOrContains($entity)) {
                    return true;
                }
            }
        }
